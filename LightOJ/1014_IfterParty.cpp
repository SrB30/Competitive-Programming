...
void checkdiv(ll p, ll n, ll l){
    ll sq = sqrt(n);
    vl ans;
    forn(i,1,sq){
        //dbg(i);
        if(n%(ll)i==0 && p%(ll)i==l) ans.pb((ll)i);
        if((ll)i!=(n/(ll)i) && n%(ll)(n/i)==0 && p%(ll)(n/i)==l) ans.pb((ll)(n/i));
    }
    if((int)ans.size()<1) {cout << "impossible\n";}
    else{sort(all(ans)); cout << ans[0];
    for(int i=1; i<(int)ans.size(); i++) cout <<" "<< ans[i]; nl;}
}
